...
La machine ayant servit à l'exécution des deux codes est composée d'un processeur Ryzen 9 7950X3D, d'une carte graphique Nvidia RTX 4080 et de 16 Go de mémoire vive.
...
En effet, le code de FluidX3D est exécuté en $\approx 3$ secondes alors que le code de GVDB-Voxel est exécuté en environ $\approx 30$ secondes.

Cette différence est crutial et semble indiquer clairement quel code est le plus efficace.
...
La différence observée peut également être dûe au choix de la méthode, la méthode lbm étant peut-être objectivement plus rapide que la méthode sph, à l'implémentation qui dans le cas de FluidX3D reçoit régulièrement des mises à jour, des optimisations ou encore des ajouts de fonctionnalité tandis que dans le cas de GVDB-Voxel, aucune mise à jour n'est faite, ce qui peut signifier que soit le code est parfait, ce qui serait étonnant vu les difficultés rencontrées à l'initialisation de la méthode SPH et vu le peu de documentation expliquant les paramètres utilisés dans le code, soit le code est certe un bon code, mais pas forcément très optimisé ni sans erreurs.
