# Méthode LBM {#sec-methode-lbm}

## Histoire {#sec-lbmh}

La "Lattice Boltzman Method" (LBM) est basée sur la méthode "Lattice Gaz Automata" (LGA) développée pen 1973 ar J. Hardy, Y. Pomeau et O. de Pazzis [-@Hardy] sous le nom de méthode HPP.
Cette méthode était basée non pas sur les équations de Navier-Stokes [-@sec-NSE], mais sur les équations de Boltzmann [-@sec-BE].

En 1986, U. Frisch, B. Hasslacher et Y. Pomeau ont réussi à obtenir les équations de Naiver-Stokes à partir de la méthode LGA en utilisant une lattice hexagonale [-@Frisch].
La méthode LGA rencontrait des difficultés, notamment un bruit statistique causé par les approximations de la méthode LGA.

Pour pallier à ce problème, G. McMamara et G. Zanetti ont décidé de négliger les corrélations entre les particules et ont intégré la notion de fonction de distribution moyenne dans leur travail [-@McMamara], ce qui a donné naissance à la méthode LBM.
Puis en 1989, Higuera et Jimenez ont simplifié la méthode LBM en définissant un opérateur de collision linéaire [-@Higuera].
Et enfin, un opérateur de collision basé sur le travail de Bhatnagar, Gross et Krook [-@Bhatnagar] a été utilisé simultanément par Koelman [-@Koelman] et par Chen et al. [-@Chen].

De nos jours, la méthode LBM est devenue populaire car elle présente une nouvelle approche efficace dans le domaine "Computational Fluid Dynamics" (CFD) carj elle ne cherche pas à résoudre directement les équations de Navier-Stokes [-@sec-NSE], contrairement à la méthode SPH.
...
Le but est alors de savoir comment vont évoluer ces particules dans la grille, comment celles-ci vont interagir entre elles, quel est leur mouvement moyen dans le système et quelle est leur vitesse.

Dans les équations de Navier-Stokes [-@sec-NSE], il n'y a que 2 variables, soit la position et le temps, tandis que dans la méthode LBM, la vélocité des molécules est également perçue comme une variable qui pourra être nommée $\xi$ et qui décrit le changement de position $\mathbf{x}$ de la molécule en fonction du temps $t$, comme nous le montre l'équation [-@eq-xi].
...
Ce qui est tout particulièrement intéressant est de calculer les moments de la distribution $f$ en fonction de la vitesse des molécules $\xi$.
Cela permet de relier la description à l'échelle des grouptes de molécules, c'est-à-dire la description mésoscopique, à la description macroscopique du fluide telle que la densité, la pression ou encore la vitesse du fluide.
En effet, les propriétés globales du fluide sont dégagées grâce au suivi des contributions de chaque groupe de molécules.

#### Moment d'ordre $0$

Le moment d'ordre $0$ par rapport à $\xi$ peut se calculer comme dans l'équation [-@eq-mo0].
Il permet d'obtenir la densité $\rho$ du fluide à une position $\mathbf{x}$ et à un temps $t$ car l'intégration en fonction de la vitesse des particules a pour résultat de prendre chaque particule sans tenir compte de leur vitesse.
Ce qui donne la densité de particules en une position $\mathbf{x}$ pour un temps $t$.
...
Le moment d'ordre $1$ par rapport à $\xi$ de la fonction de distribution $f$ peut s'obtenir comme montré dans l'équation [-@eq-mo1].
...
En d'autres termes, cela donne la vitesse macroscopique du fluide en une position et à un temps donnés.
...
Cependant, grâce au travail de Bhatnagar, Gross et Krook (BGK) [-@Bhatnagar], comme le système va tendre vers un état équilibré $f^{eq}$ à partir de l'état non équilibré $f$, l'opérateur de collision peut être simplifié en la relaxation de $f$ vers $f^{eq}$ comme l'équation [-@eq-bgk] le démontre, avec $\tau$ le temps de relaxation.
...
Dans cette partie, la définition $\mathbf{v} = \xi - u$ est la vitesse du centre de masse d'une population de particules et les collisions entre particules sont considérées comme étant élastiques ce qui signifie que la masse, le moment et l'énergie sont conservés.
...
- $\mathbf{x} \rightarrow \Delta \mathbf{x}$ avec $\Delta \mathbf{x}$ qui est l'espace occupé par une population. Ainsi, l'espace est divisé en plusieurs noeuds chaque noeud représentant l'espace occupé par une population.
  Par exemple dans la figure [-@fig-pop], $\Delta x$ sera un carré de la grille.
- $t \rightarrow \Delta t$ avec $\Delta t$ qui est en général un intervale de temps fixé, bien qu'il soit possible de modifier dynamiquement $\Delta t$.
- $f(\mathbf{v}, \mathbf{x}, t) \rightarrow f_{i}(\mathbf{x}, t)$ pour la vélocité $\mathbf{v}$. En fait, $\mathbf{v}$ est un vecteur.
  ...
  C'est pourquoi, la discrétisation de la vélocité consiste à déterminer un nombre limité de directions et de valeurs pour le vecteur vitesse $\mathbf{v}$.
  Par exemple, la figure [-@fig-vel] illustre les vélocités que peut prendre une population.
  La notation D2Q9 indique que l'espace possède 2 dimensions et que seulement 9 valeurs de vélocité sont possibles pour une population.
...
Les discrétisations introduites permettent de discrétiser également la fonction de distribution équilibrée comme montré dans l'équation [-@eq-fieq] avec $w_{i}$ le poids d'une population qui dépend du déplacement ou de l'immobilité de cette dernière. 
...
### Lattice Boltzmann Equation (LBE) {#sec-LBE}
...
- La distribution équilibrée peut donc être calculée grâce à la condition initiale $f_{i}$ car elle ne dépend que de la densité et de la vélocité, qui ont pu être calculées précédemment.
...
- La masse et la quantité de mouvement sont conservées car : $\sum_{i}\Omega_{i} = 0$ et $\sum_{i} c_{i}\Omega_{i} = 0$.
- Les analyses de Chapman-Enskog [-@Chapman] et [-@Enskog] montrent que les LBE [-@sec-LBE] sont suffisantes pour retrouver le comportement de Navier-Stokes [-@sec-NSE].
